...
/*   Updated: 2023/05/21 22:41:21 by lkavalia         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
bool	AForm::checkExecRequirements(const Bureaucrat& bureaucrat) const
{
	try
	{
		if (this->_form_signed == true)
			throw FormHasBeenSignedException();
		if (bureaucrat.getGrade() > _min_grade_to_execute || bureaucrat.getGrade() > _min_grade_to_sign)
			throw GradeTooLowException();
	}
	catch(FormHasBeenSignedException& e)
	{
	...
	{
		std::cerr << RED << bureaucrat.getGrade() << e.what() << BLANK << std::endl;
		return (false);
	}
	return (true);
}
